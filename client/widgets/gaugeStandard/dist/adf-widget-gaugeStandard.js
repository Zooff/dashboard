...
$templateCache.put("{widgetsPath}/gaugeStandard/src/view/view.html","<div><div ng-hide=graph.data class=\"alert alert-info\" role=alert>Please insert a url to the widget configuration</div><div ng-show=graph.data>{{config.urlReplace}}<div class=text-center><ng-gauge type={{graph.config.type}} size=200 thick=5 value=graph.data.servers.length cap=round label={{graph.config.label}} append={{graph.config.append}} min=graph.config.min max=graph.data.max></ng-gauge></div><div class=text-center><p>{{graph.config.desc}}</p></div></div></div>");}]);
...
gaugeStandardController.$inject = ["$scope", "data", "gaugeStandardService", "$rootScope", "$uibModal"];
